...
BATCH_SIZE = [1, 2, 4, 8]
BACK_END = ['pytorch', 'openvino', 'tensorflow'] # plot_results logic only considering pytorch and openvino
...
def plot_results(summary_df):
    ...
    for seq_len in SEQ_LENGTH:
        for no_ins in NUM_OF_INSTANCE:
            for bs in BATCH_SIZE:
                df = summary_df.loc[(summary_df['batch_size'] == bs) & (summary_df['seq_len'] == seq_len) & \
                ...
                df_.loc[df_.index.max() + 1, :] = [row.loc['seq_len'], row.loc['instance_id'], \
                                                   row.loc['batch_size'], row.loc['num_threads'], \
                                                   pt_latency, ov_latency]
